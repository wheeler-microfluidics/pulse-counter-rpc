...
following command:
    ...
    python -m od_sensor_rpc.bin.upload <board type>
...
argument to specify the serial port to use.  For example:
    ...
    python -m od_sensor_rpc.bin.upload -p COM3 uno

--------------------------------------------------

## Usage ##

After uploading the firmware to the board, the `od_sensor_rpc.Proxy` class can be
...
### Example interactive session ###
    ...
    >>> from od_sensor_rpc import Proxy
...
Below is a list of the attributes of the `od_sensor_rpc.Proxy` Python class.  Note
...
The Arduino firmware/sketch is located in the `od_sensor_rpc/Arduino/od_sensor_rpc`
directory.  The key functionality is defined in the `od_sensor_rpc::Node` class in
the file `Node.h`.
...
`od_sensor_rpc::Node` class in the file `Node.h`.
